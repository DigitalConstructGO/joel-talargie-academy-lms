...
async function issueCertificateForEnrollment() {
  ...
  const tplRes = await pool.query(
    'SELECT id, name, version, configuration FROM certificate_templates WHERE is_default = true LIMIT 1',
  );
  ...
  `,
    [
      `certificate:generate:${cert.id}:v1`,
      JSON.stringify({ certificateId: cert.id, enrollmentId }),
    ],
  );
